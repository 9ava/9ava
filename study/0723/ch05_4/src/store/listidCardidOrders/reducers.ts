...
export const reducer = (state: T.State = initialState, action: T.Actions) => {
  switch (action.type) {
    ...
    case '@listidCardids/remove': {
      ...
      delete newState[action.payload as string] // 색인 연산자로 대상 속성 지정 후 delete
      return newState
    }
    case '@listidCardids/prependCardid': {
      const cardids = state[action.payload.listid as string]
      return {...state, [action.payload.listid as string]: [action.payload.cardid, ...cardids]}
    }
    case '@listidCardids/appendCardid': {
      // 계산된 속성명은 추가 뿐만 아니라 update도 가능하다.
      const cardids = state[action.payload.listid as string]
      return {...state, [action.payload.listid as string]: [...cardids, action.payload.cardid]}
    }
    case '@listidCardids/removeCardid': {
      const cardids = state[action.payload.listid as string]
      return {
        ...state,
        [action.payload.listid as string]: cardids.filter(id => id !== action.payload.cardid)
      }
    }
  }
  return state
